...
export default function ImageCarousel({ images }) {
  ...
  return (
    <motion.div className="relative w-full object-cover h-full">
      ...
      <motion.button
        onClick={handleNext}
        className="absolute text-xl sm:text-2xl transition-all duration-300 text-gray-400 hover:text-gray-400 right-0 top-1/2 transform -translate-y-1/2 cursor-pointer z-20"
      ...
      <motion.button
        onClick={handlePrev}
        className="absolute text-lg sm:text-2xl transition-all duration-300 text-gray-400 hover:text-gray-400 left-0 top-1/2 transform -translate-y-1/2 cursor-pointer z-20"
        initial={{ opacity: 0 }} // Tombol dimulai dengan opacity 0
        animate={{ opacity: isAnimating ? 0 : 1 }} // Tombol hanya muncul setelah animasi selesai
